...
const sampleUsers: User[] = [
  ...
  ...Array.from({ length: 45 }, (_, i) => ({
    ...
    avatar: i % 4 === 0 ? null : `https://images.unsplash.com/photo-${1500000000000 + i}?w=100&h=100&fit=crop&crop=face`
  }))
];
...
// Componente Badge personalizado para estados
const StatusBadge: React.FC<{ status: User['status'] }> = ({ status }) => {
  const getStatusConfig = (status: User['status']) => {
    switch (status) {
      case 'active':
        return { text: 'Activo', color: '#10b981', bg: '#dcfce7' };
      case 'inactive':
        return { text: 'Inactivo', color: '#6b7280', bg: '#f3f4f6' };
      case 'pending':
        return { text: 'Pendiente', color: '#f59e0b', bg: '#fef3c7' };
      default:
...
const meta: Meta<typeof Grid<User>> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'Componente Grid moderno con paginador profesional para el tema light. Incluye funcionalidades completas de tabla y paginación.'
      }
    }
  },
  decorators: [
